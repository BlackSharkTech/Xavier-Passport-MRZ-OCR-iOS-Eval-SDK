 ...
 setOnlyNonDebugParams:(BOOL)setOnlyNonDebugParams
{
    ...
    if (self) {
        ...
        _tesseract->SetPageSegMode(tesseract::PSM_AUTO);
...
- (void)copyDataToDocumentsDirectory
{
    ...
    NSString *documentPath = ([documentPaths count] > 0) ? documentPaths[0] : nil;
...
/**
 * getRecognizedCGRectBoxes
 */
-(NSMutableArray *) getRecognizedCGRectBoxes
{
    NSMutableArray* boxArray = [[NSMutableArray alloc] init];
    
    Pixa* pixa;
    int *blockids;
    Boxa* boxa = _tesseract->GetStrips(&pixa, &blockids);
    ...
    for (int i = 0; i < count; i++)
    {
        ...
        [boxArray addObject:[NSValue valueWithCGRect:CGRectMake(box->x,box->y,box->w,box->h)]];
    }
    //boxDestroy(&box);
    boxaDestroy(&boxa);
